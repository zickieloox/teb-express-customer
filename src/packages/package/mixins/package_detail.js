...
export default {
  computed: {
    isEmpty() {
      return isEmpty(this.currentPackage)
    },
    currentPackage() {
      return this.package_detail.package || {}
    },
    hasCancelPackage() {
      const { status_string } = this.currentPackage
      return (
    ...
    hasReshipPackage() {
      const { status_string, alert } = this.currentPackage
      return (
        status_string === PackageStatusPendingPickupText &&
    ...
    linkTrackInfo() {
      const { tracking_number } = this.currentPackage
